...
class CFG:
    """A1: full-backbone ResNet50 continuation from FoodLens FT-V2."""

    RUN_ID: str = "a1_resnet50_ft_v3_full_backbone_224"
    SEED: int = 42
    NUM_CLASSES: int = 101
    ...
    CHAMPION_ARTIFACT_DIR: Path = Path(
        "/kaggle/input/models/tuannm3823/food101-resnet50-refinements/"
        "pytorch/default/1"
    )
    CHAMPION_CHECKPOINT_NAME: str = "resnet50_ft_v2_best.pth"
...
def resolve_checkpoint() -> Path:
    candidates = [
        CFG.CHAMPION_ARTIFACT_DIR / CFG.CHAMPION_CHECKPOINT_NAME,
        CFG.CHAMPION_ARTIFACT_DIR
        / "food101-resnet50-refinements"
        / CFG.CHAMPION_CHECKPOINT_NAME,
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    if CFG.CHAMPION_ARTIFACT_DIR.exists():
        matches = sorted(CFG.CHAMPION_ARTIFACT_DIR.rglob(CFG.CHAMPION_CHECKPOINT_NAME))
        if matches:
            return matches[0]

    searched = "\n".join(str(candidate) for candidate in candidates)
    raise FileNotFoundError(
        f"Could not find {CFG.CHAMPION_CHECKPOINT_NAME}. Searched:\n{searched}"
    )
...
best_checkpoint_path = RESULTS_DIR / "resnet50_ft_v3_full_backbone_best.pth"

print(f"Loaded champion checkpoint: {checkpoint_path}")
print(
    ...
    [
        {
            ...
            "latency_ms_per_image": 5.35,
        },
        {
            "model": "A1 ResNet50 FT-V3 full backbone",
...
comparison_df["test_top_1_delta"] = (
